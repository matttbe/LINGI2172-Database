...
TYPE DATEDAY  POSSREP {DATEDAY INTEGER};
...
VAR CAR           BASE RELATION {HEIGHT INTEGER, FUEL CHAR, USERID U#, LATITUDE RATIONAL, LONGITUDE RATIONAL}  KEY {};
...
VAR OPENINGHOURS  BASE RELATION {DAYSTART DATEDAY, DAYEND DATEDAY, HOURSTART CHAR, HOUREND CHAR, PARKINGID P#}  KEY {};
...
CONSTRAINT C12 IS_EMPTY (OPENINGHOURS{DAYSTART} WHERE (OPENINGHOURS{DAYSTART} < 0 OR OPENINGHOURS{DAYSTART} > 6);
CONSTRAINT C13 IS_EMPTY (OPENINGHOURS{DAYEND} WHERE (OPENINGHOURS{DAYEND} < 0 OR OPENINGHOURS{DAYEND} > 6);
CONSTRAINT C14 IS_EMPTY (PARKING WHERE PARKING{FREEPLACES} > PARKING{TOTALPLACES});
CONSTRAINT C15 IS_EMPTY (PARKING{HEIGHT} WHERE PARKING{HEIGHT} < 0);
CONSTRAINT C16 IS_EMPTY (CAR{HEIGHT} WHERE CAR{HEIGHT} < 0);
